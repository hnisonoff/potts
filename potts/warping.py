...
class TanhWarpingLayer(torch.nn.Module):
    ...
    def set_parameters(self, a, b, c, d):
        ...
        """
        if len(a) != self.num_warping_terms or len(
                b) != self.num_warping_terms or (len(c) !=
                                                 self.num_warping_terms):
            raise ValueError("Expected %d warping terms",
                             self.num_warping_terms)
